...
assert (exe == null) || builtins.isString exe; let
  envPairs =
  ...
  exePath =
    if exe != null
    then "${package}/bin/${exe}"
    else if package ? meta && package.meta ? mainProgram && package.meta.mainProgram != null
    then "${package}/bin/${package.meta.mainProgram}"
